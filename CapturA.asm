...
; «««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««
    ...
    GetRegistryMouseMenu        PROTO :DWORD,:DWORD ;szMouseHoverTime, szMenuShowDelay
...
; «««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««
    ...
    IDC_PROMPT_OPEN_LAST_CAPTURE_PATH   equ 039
    IDC_CHK_OPEN_LAST_CAPTURE_PATH      equ 040
    ...
    IDC_PROMPT_COPYTOCLIPBOARD          equ 066
    IDC_BTN_CHECK_COPYTTOCLIPBOARD      equ 067
    IDC_PROMPT_SHOW_NOTIFICATION_ICON   equ 068
    IDC_CHK_SHOW_NOTIFICATION_ICON      equ 069
    IDC_PROMPT_DISPLAY_NOTIFICATION_CAPTURE equ 070
    IDC_CHK_DISPLAY_NOTIFICATION_CAPTURE equ 071
    IDC_PROMPT_MINIMIZE_AT_SYSTEM_TRAY  equ 072
    IDC_CHK_MINIMIZE_AT_SYSTEM_TRAY     equ 073
    ;
    ...
    ;
    IDC_SHEET_TAB_CONTROL_COVER         equ 092
    ;
    ...
    IDM_ABOUT                           equ 102
    IDM_EXIT                            equ 103
    HOTKEY_SHIFT_CTRL_ALT_PRINTSCREEN   equ 104
    HOTKEY_SHIFT_CTRL_ALT_S             equ 105
    HOTKEY_PRINTSCREEN                  equ 106
    HOTKEY_ALT_PRINTSCREEN              equ 107
    HOTKEY_CTRL_PRINTSCREEN             equ 108
    HOTKEY_CTRL_ALT_PRINTSCREEN         equ 109
    HOTKEY_SHIFT_CTRL_E                 equ 110
    HOTKEY_SHIFT_CTRL_ALT_E             equ 111
...
; «««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««
   ...
   ;
    ...
    bAutoShotStartAtInit_RunIt  db ? ;Auxiliar to start
    bAutoShotStartAtInit_IconIt db ? ;Auxiliar to start
    ;
    ...
    bMinimizeWindowIntoNotificationIcon     db ?
...
RunCmdAtMainEntry               proc
    ...
    ;************ Get Cursor ******
    szText szCursorName,"cursor.cur"
    ...
    inc eax
    mov byte ptr [eax],0
    Invoke lstrcat,addr szCurFile,addr szCursorName
...
;        ICC_FLAGS = ICC_FLAGS or ICC_USEREX_CLASSES
    ...
    .else
        ...
        ;pop eax
        .if eax==0
            .if hSingleInstanceAtom == 0
                jmp @B
            .endif
        .elseif eax==2 ;The system cannot find the spefified file
            jmp @B
        .endif
        ;Invoke MessageBox,NULL,addr szProgramPath,addr szProgramPath,NULL
        ;Invoke ErrorManager, addr szAppTitle
        ;Invoke GetLastError
        ;.if eax==2 ;The system cannot find the spefified file
        ;    jmp @B
        ;.endif
        ;Invoke ErrorManager, addr szAppTitle
        ;Invoke wsprintf,addr szBuffer,addr szIntFormat,eax
        ;Invoke MessageBox,NULL,addr szBuffer,addr szAppTitle,MB_ICONWARNING
        ...
        .if eax!=0
            ...
            .if hPreviousWin
                ;Invoke MessageBox,NULL,addr szProgramPath,addr szProgramPath,NULL
                Invoke ShowWindowAsync,hPreviousWin, SW_SHOWNORMAL or SW_RESTORE
                Invoke SetForegroundWindow, hPreviousWin
                ;Invoke ShowWindowAsync,hWinInTurn, SW_SHOWNORMAL or SW_RESTORE
                ;Invoke SetForegroundWindow,hWinInTurn
            ...
            .endif
            ;szText szAlreadyRunningTitle,"The application is already running."
            ;szText szAlreadyRunningText,"Another instance of the application is already Running",10,10,\
            ;"Or the previous intance crashed / closed improperly :(",10,10,"If this is a false warning, do You want to clear the Already Running Flag?"
            ;Invoke MessageBox,NULL,addr szAlreadyRunningText,addr szAlreadyRunningTitle,MB_ICONWARNING+MB_YESNOCANCEL ;MB_ICONQUESTION
            ;.if eax==IDYES;IDOK
            ;    Invoke GlobalDeleteAtom,hSingleInstanceAtom
            ;.endif
            ;m2m eax, ERROR_ALREADY_EXISTS
            ;
            xor eax,eax
            inc eax
                Invoke MessageBoxA,NULL,addr szAtom,NULL,NULL
...
WinMain                         proc
    ...
    pop [eax]
            ...
            ;WS_OVERLAPPED OR WS_POPUP OR WS_VISIBLE OR WS_CAPTION OR WS_SYSMENU OR DS_CENTER OR WS_EX_TOPMOST WS_EX_APPWINDOW +WS_EX_APPWINDOW
            ;WS_VISIBLE+
    Dialog  "CapturA+: Screen Capture & SnapShot Recorder","Segoe UI",9,\
            WS_OVERLAPPED+DS_CENTER+WS_POPUP+WS_CAPTION+WS_SYSMENU+WS_MINIMIZEBOX,\
            88, \                                            ; control count
            0,0,540,298, \                                   ; x y co-ordinates
            5096                                             ; memory buffer size
            ...
            DlgStatic "Open Last Capture Location Path After Capture",WS_GROUP OR WS_CHILD OR WS_VISIBLE,204+12,40,312,10,IDC_PROMPT_OPEN_LAST_CAPTURE_PATH
            DlgButton "Off",WS_CHILD OR WS_VISIBLE OR WS_TABSTOP OR BS_PUSHLIKE OR BS_AUTOCHECKBOX,204+12,52,54,12,IDC_CHK_OPEN_LAST_CAPTURE_PATH
            ...
            DlgStatic "Hide Application Window When Opening",WS_GROUP OR WS_CHILD OR WS_VISIBLE,204+12,94,312,10,       IDC_PROMPT_HIDE_WIN_WHEN_OPENING
            ...
            DlgStatic "Copy Screenshot to Clipboard After Capture",WS_GROUP OR WS_CHILD OR WS_VISIBLE,204+12,158,312,10,IDC_PROMPT_COPYTOCLIPBOARD
            DlgButton "Off",WS_CHILD OR WS_VISIBLE OR WS_TABSTOP OR BS_PUSHLIKE OR BS_AUTOCHECKBOX,204+12,170,54,12,    IDC_BTN_CHECK_COPYTTOCLIPBOARD
            DlgStatic "Show A Notification Icon",WS_GROUP OR WS_CHILD OR WS_VISIBLE,204+12,190,312,10,                  IDC_PROMPT_SHOW_NOTIFICATION_ICON
            DlgButton "Off",WS_CHILD OR WS_VISIBLE OR WS_TABSTOP OR BS_PUSHLIKE OR BS_AUTOCHECKBOX,204+12,202,54,12,    IDC_CHK_SHOW_NOTIFICATION_ICON
            DlgStatic "Display A Notification After Capture",WS_GROUP OR WS_CHILD OR WS_VISIBLE,204+12,222,312,10,      IDC_PROMPT_DISPLAY_NOTIFICATION_CAPTURE
            DlgButton "Off",WS_CHILD OR WS_VISIBLE OR WS_TABSTOP OR BS_PUSHLIKE OR BS_AUTOCHECKBOX,204+12,234,54,12,    IDC_CHK_DISPLAY_NOTIFICATION_CAPTURE
            DlgStatic "Minimize Window Into Notification Icon",WS_GROUP OR WS_CHILD OR WS_VISIBLE,204+12,254,312,10,    IDC_PROMPT_MINIMIZE_AT_SYSTEM_TRAY
            DlgButton "Off",WS_CHILD OR WS_VISIBLE OR WS_TABSTOP OR BS_PUSHLIKE OR BS_AUTOCHECKBOX,204+12,266,54,12,    IDC_CHK_MINIMIZE_AT_SYSTEM_TRAY
            ...
            DlgEdit  ES_READONLY OR WS_CHILD OR WS_VISIBLE OR WS_BORDER OR WS_TABSTOP OR ES_AUTOHSCROLL,204+12,266,180,12,  IDC_EDITBOX_HOTKEYS_EMAIL_TOPWIN
            ;
            ;===============================================
            ;          propertySheet
...
WinMainProc                     proc hWnd:DWORD,uMsg:DWORD,wParam:DWORD,lParam:DWORD
    ...
    .if uMsg == WM_INITDIALOG
        ...
        .if bHideApplicationWindowWhenOpening==FALSE
        .endif
        ;
    ...
    .elseif uMsg == WM_SETFOCUS
        ...
        .if bAutoShotStartAtInit_RunIt==TRUE ;First time?
            mov bAutoShotStartAtInit_RunIt,FALSE
            mov bAutoShotStartAtInit_IconIt,TRUE
            ;
            .if bHideApplicationWindowWhenOpening
                ...
                ;Invoke SendMessage,hWinMain,WM_SYSCOMMAND,TRUE,SW_PARENTCLOSING ;TRUE=SHOW FALSE=HIDE
                ;Invoke SendMessage,hWinMain,WM_SHOWWINDOW,FALSE,SW_PARENTCLOSING ;TRUE=SHOW FALSE=HIDE
...
;        Invoke RedrawWindow,0,addr WinMainRec,0,RDW_INVALIDATE OR RDW_ALLCHILDREN OR RDW_UPDATENOW
    ...
    .elseif uMsg == WM_SIZE
        ...
        .if wParam==SIZE_MINIMIZED
            .if bAutoShotStartAtInit_IconIt==TRUE
                mov bAutoShotStartAtInit_IconIt,FALSE
...
;                .endif
            .else
                Invoke IniManager,hWnd,2   ;(WinHandle),(0=GET,1=PUTMAIN,2=READMAIN,3=PUTSECURITY,4=READREGION,5=PUTREGION,6=READCOUNTER,7=PUTCOUNTER)
                ;================================================
                ; Hide Window
                ;================================================
                .if bShowANotificationIcon==FALSE
                    Invoke ShowWindowAsync,hWinMain,SW_HIDE
                    Invoke Shell_NotifyIcon, NIM_DELETE,ADDR TrayBalloonNote
                .else
                    ;&& bMinimizeWindowIntoNotificationIcon
                    ;================================================
                    ; Tray Icon
                    ;================================================
                    .if bShowANotificationIcon
    ...
    .elseif uMsg==WM_SHELLNOTIFY ; Icon on system tray recieved a message
        .if wParam==IDM_TRAY
            .if lParam==WM_RBUTTONDOWN ; Check if the user has right-clicked
                ...
                Invoke TrackPopupMenu,hTrayPopupMenu,TPM_BOTTOMALIGN+TPM_RIGHTALIGN+TPM_RETURNCMD, pnt.x,pnt.y,0,hWnd,0
                ...
                .if ax==IDM_RESTORE
                    ...
                    .if dFirstTimeShowed==FALSE
                        ...
                        ;Invoke SetWindowPos,hWnd,HWND_TOPMOST,WinPos.left,WinPos.top,WinPos.right,WinPos.bottom,SWP_SHOWWINDOW ;SWP_HIDEWINDOW OR SWP_NOMOVE OR SWP_NOSIZE OR SWP_NOREPOSITION OR SWP_NOACTIVATE
                        Invoke Sleep,20
                        Invoke IsIconic,hWinMain
                        .if eax
                            Invoke ShowWindow,hWinMain,SW_RESTORE
                        .endif
                        Invoke SetWindowPos,hWnd,HWND_TOP,WinPos.left,WinPos.top,WinPos.right,WinPos.bottom,SWP_SHOWWINDOW ;SWP_HIDEWINDOW OR SWP_NOMOVE OR SWP_NOSIZE OR SWP_NOREPOSITION OR SWP_NOACTIVATE
                        Invoke Sleep,20
                        Invoke SetWindowPos,hWnd,HWND_NOTOPMOST,WinPos.left,WinPos.top,WinPos.right,WinPos.bottom,SWP_SHOWWINDOW ;SWP_HIDEWINDOW OR SWP_NOMOVE OR SWP_NOSIZE OR SWP_NOREPOSITION OR SWP_NOACTIVATE
                    .else
                        Invoke IsIconic,hWinMain
                        .if eax
                            Invoke ShowWindow,hWinMain,SW_RESTORE
...
;            .endif
        .elseif wParam == IDC_BTN_DIALOG_CAPTURE_REGION               ;Region Setting
            ...
            .endif
        .elseif wParam == IDC_BTN_SAVE                  ;Save Settings
            shr eax,16
            .if ax==BN_CLICKED
                  Invoke IniManager,hWnd,1   ;(WinHandle),(0=GET,1=PUTMAIN,2=READMAIN,3=PUTSECURITY,4=READREGION,5=PUTREGION,6=READCOUNTER,7=PUTCOUNTER)
            .endif
...
;            Invoke IniManager,hWnd,2   ;(WinHandle),(0=GET,1=PUTMAIN,2=READMAIN,3=PUTSECURITY,4=READREGION,5=PUTREGION,6=READCOUNTER,7=PUTCOUNTER) ;Button "_" & "X"
...
;            .endif
    ...
    ;>>>>>>>>>>>>>> 7. TRAY ICON = Remove tray icon, before close >>>>>>>>>>>>>
        Invoke Shell_NotifyIcon, NIM_DELETE,addr TrayBalloonNote
        Invoke DestroyMenu,hTrayPopupMenu
    ;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ;
...
InitDialogWinMain               proc hWnd:DWORD
        ...
        Invoke SendMessage,eax,BM_SETCHECK,BST_CHECKED,NULL
        ;
        ;Invoke GetDlgItem,hWnd,IDC_BTN_SAVE
...
;        Invoke ShowWindow,eax,SW_HIDE
        ...
        .if bHideApplicationWindowWhenOpening
            ...
            ;
            Invoke GetWindowRect,hWnd,addr WinPos
            mov eax,WinPos.right
            sub eax,WinPos.left
            ;mov nWidth,eax
            mov ecx,WinPos.bottom
            sub ecx,WinPos.top
            ;mov nHeight,eax
            mov WinPos.right,eax
            mov WinPos.bottom,ecx
            ;
            Invoke GetWindowLong,hWnd,GWL_EXSTYLE
            and eax,WS_EX_APPWINDOW ;+WS_EX_TOPMOST
            ;or eax,WS_EX_WINDOWEDGE
            or eax,WS_EX_TOOLWINDOW
            Invoke SetWindowLong,hWnd,GWL_EXSTYLE,eax
            ;Invoke ShowWindow,hWnd,SW_MINIMIZE
            ;            
            ;
            Invoke SetWindowPos,hWnd,HWND_BOTTOM,0,0,0,0,SWP_HIDEWINDOW OR SWP_FRAMECHANGED
            ;
;            Invoke SetWindowPos,hWnd,HWND_BOTTOM,0,0,,0,SWP_HIDEWINDOW
;            ;
;            Invoke GetWindowLong,hWnd,GWL_EXSTYLE
;            or eax,WS_EX_APPWINDOW
;            ;or eax,WS_EX_WINDOWEDGE
;            and eax,WS_EX_TOOLWINDOW
;            Invoke SetWindowLong,hWnd,GWL_EXSTYLE,eax           
            ;Invoke SendMessage,hWinMain,WM_SHOWWINDOW,FALSE,SW_PARENTCLOSING ;TRUE=SHOW FALSE=HIDE            

;            Invoke GetWindowLong,hWnd,GWL_EXSTYLE
;            ;.if (eax & WS_EX_APPWINDOW)
;                and eax,WS_EX_APPWINDOW
;                Invoke SetWindowLong,hWnd,GWL_EXSTYLE,eax
;                ;Invoke SetLayeredWindowAttributes,lParam,0,200,LWA_ALPHA
;            ;.endif

            ;Invoke SetWindowPos,hWnd,HWND_NOTOPMOST,WinPos.left,WinPos.top,WinPos.right,WinPos.bottom,SWP_SHOWWINDOW ;SWP_HIDEWINDOW OR SWP_NOMOVE OR SWP_NOSIZE OR SWP_NOREPOSITION OR SWP_NOACTIVATE
;            ;Invoke SetWindowPos,hWnd,NULL,Rct.top,Rct.left,Rct.right,Rct.bottom,SWP_SHOWWINDOW
;            ;Invoke SetWindowPos,hWnd,NULL,0,0,0,0,NULL
;            ;Invoke SetWindowPos,hWnd,HWND_NOTOPMOST,0,0,0,0,SWP_HIDEWINDOW OR SWP_NOMOVE OR SWP_NOSIZE OR SWP_NOREPOSITION OR SWP_NOACTIVATE
;;            ;Invoke SetWindowPos,hWnd,HWND_BOTTOM,0,0,0,0,SWP_HIDEWINDOW OR SWP_NOMOVE OR SWP_NOSIZE OR SWP_NOREPOSITION
;;            ;Invoke SetWindowPos,hWnd,NULL,0,0,0,0,SWP_HIDEWINDOW OR SWP_NOMOVE OR SWP_NOSIZE OR SWP_NOREPOSITION
;;            ;Invoke SetWindowPos,hWnd,NULL,0,0,0,0,SWP_HIDEWINDOW OR SWP_NOMOVE OR SWP_NOSIZE
;;            ;Invoke SetWindowPos,hWnd,NULL,0,0,0,0,NULL
...
;            ;Invoke ShowWindow,hWnd,SW_HIDE
       ...
       ;
        ...
        szText szRestore,"Restore Window"
        szText szAbout,"About CapturA+"
        szText szExit,"Close CapturA+"
        Invoke AppendMenu,hTrayPopupMenu,MF_STRING,IDM_RESTORE,addr szRestore
        Invoke AppendMenu,hTrayPopupMenu,MF_STRING,IDM_ABOUT,addr szAbout
        Invoke AppendMenu,hTrayPopupMenu,MF_SEPARATOR, 0, 0
        Invoke AppendMenu,hTrayPopupMenu,MF_STRING,IDM_EXIT,addr szExit
        ;
        ...
        mov TrayBalloonNote.uID,IDM_TRAY
...
;        Invoke SendMessage, hCombo2, CB_SETCURSEL, eax, 0
        ...
        .if hCursor
            lea ecx, szCursorName
        .else
            lea ecx, szCaptureCursor
            add ecx,15   ;szCaptureCursor             db 45 dup('Disable',0,0,0,0,0,0,0,0,'Default Cursor',0,'Red Arrow',0,0,0,0,0,0);15
        .endif
      ...
      ;
        ...
        ;
        .if bCaptureMode==RUNTIME_CAPTURE_MODE_AUTOSHOT && bAutoShotStartAtInit==TRUE
            mov bAutoShotStartAtInit_RunIt,TRUE
        .else
            mov bAutoShotStartAtInit_RunIt,FALSE
        .endif
         mov bAutoShotStartAtInit_IconIt,FALSE
        ...
        ;
        Invoke GetDlgItem,hWnd,IDC_BTN_CHECK_COPYTTOCLIPBOARD
        ...
        Invoke  SendMessage, eax, BM_SETCHECK,ecx,NULL
        ;
        ;*************** Initialize Check Control for bMinimizeWindowIntoNotificationIcon ******************
        ;
        Invoke GetDlgItem,hWnd,IDC_CHK_MINIMIZE_AT_SYSTEM_TRAY
        .if bMinimizeWindowIntoNotificationIcon
            mov ecx,BST_CHECKED
        .else
            mov ecx,BST_UNCHECKED
        .endif
...
SetTheme                        proc hWnd:DWORD,pFonts:DWORD
            ...
            Invoke RedrawButton,hDummy,addr BtnStruct
            ;
            mov hDummy,$invoke(GetDlgItem,hWnd,IDC_BTN_CHECK_COPYTTOCLIPBOARD)
            Invoke RedrawButton,hDummy,addr BtnStruct
            ;
            ...
            Invoke RedrawButton,hDummy,addr BtnStruct
            ;
            mov hDummy,$invoke(GetDlgItem,hWnd,IDC_CHK_MINIMIZE_AT_SYSTEM_TRAY)
            Invoke RedrawButton,hDummy,addr BtnStruct
            ;*************** Title / Subtitle ******************
...
ShowPage                        proc dPageID:DWORD,nCmdShow:DWORD
        ...
        .elseif dPageID == IDC_PAGE03
            ...
            @@:
                ...
                pop ecx
                add ecx,1
                cmp ecx,IDC_CHK_MINIMIZE_AT_SYSTEM_TRAY
                jle @B
...
;                            } SHFILEOPSTRUCT, *LPSHFILEOPSTRUCT;
        ...
        ;Invoke MessageBox,NULL,addr szSolvedJpgPath,addr szSolvedJpgComments,MB_OK
        mov dBytesWritten,0
        mov al,bImageMode
        dec al
        mov bGray,al
...
;                mov cl,byte ptr[eax]
;                .if cl==0
;                    Invoke MessageBox,NULL,addr szErrorTextB,addr szSolvedJpgPath,NULL
;                .else
;                    mov ecx,eax
;                    Invoke MessageBox,NULL,ecx,addr szSolvedJpgPath,NULL
;                .endif
            jmp ErrorShot
        .endif
        ;##########################################################
        ...
        ;                     COPY TO CLIPBOARD
        ;##########################################################
        .if bCopyScreenshotToClipboard==TRUE || dSendArchiveTo==RUNTIME_CAPTURE_SENT_TO_CLIPBOARD || dSendArchiveTo==RUNTIME_CAPTURE_SENT_TO_MAIL
            Invoke OpenClipboard,hWinMain
            .if eax==NULL ;If the function succeeds, the return value is nonzero.
                Invoke aaerrortext,eax
                ;Invoke aaerrormessage,eax
            .else
                ...
                Invoke GlobalAlloc,GMEM_MOVEABLE+GMEM_DDESHARE,dClipDataSize;dBytesWritten
                ...
                .if hClipData
                    ...
                    or eax,eax
                    jnz @F
                        Invoke SetClipboardData,CF_HDROP,hClipData
                    @@:
                .endif
                ;---------------------------------------------------------------------------
                ...
                add eax,1
                mov dClipDataSize,eax
                Invoke GlobalAlloc,GMEM_MOVEABLE+GMEM_DDESHARE,dClipDataSize
                mov hClipData, eax
                .if hClipData
                    Invoke GlobalLock,hClipData
                    mov pClipData,eax
...
AutoShotLoop                    proc dSendArchiveTo_Minutes_Seconds_OnlyOneTime:DWORD
    ...
    AutoShotLoop_NextShot:
        ...
        .endif
        ;
        Invoke IniManager,hWinMain,2   ;(WinHandle),(0=GET,1=PUTMAIN,2=READMAIN,3=PUTSECURITY,4=READREGION,5=PUTREGION,6=READCOUNTER,7=PUTCOUNTER)
...
AboutDlg                        proc hParent:DWORD,lpIcon:DWORD
    ...
    Dialog "About CapturA+","MS Sans Serif",8, \            ; caption,font,pointsize
            WS_OVERLAPPED or WS_SYSMENU or DS_CENTER, \     ; style
            3, \                                            ; control count
            0,0,268,242, \                                  ; x y co-ordinates
            1024                                            ; memory buffer size
    DlgGroup    "Programmer Stuff",8,4,248,188,IDGROUP3
    DlgStatic "[ www.carabez.com ]",WS_CHILD OR WS_VISIBLE or SS_NOTIFY,100,168,74,12,IDSTATIC12
    DlgButton   "OK",WS_TABSTOP OR 1  OR 8000H,104,200,60,16,IDC_BTN6
...
AboutDlgProc                    proc hWin:DWORD,uMsg:DWORD,wParam:DWORD,lParam:DWORD
      ...
      ;Load bitmap from resource
        Invoke BitmapFromResource, hInstance,700
        mov hBmp, eax
      ;
      ...
      ;Put File Version in Windows Title
          ...
          .if (eax != 0)
              ...
              szText szVersion," Version: "
              Invoke lstrcat,addr szBuffer,addr szVersion
              Invoke lstrcat,addr szBuffer,dDummy
...
IniManager                      proc  hWnd:DWORD, bAction:BYTE ;(WinHandle),(0=GET,1=PUTMAIN,2=READMAIN,3=PUTSECURITY,4=READREGION,5=PUTREGION,6=READCOUNTER,7=PUTCOUNTER)
                        ...
                        ;
    ;
    Invoke GetRegistryMouseMenu,addr szMouseHoverTime,addr szMenuShowDelay
    ;
    ...
    .if bAction == 0 ;(WinHandle),(0=GET,1=PUTMAIN,2=READMAIN,3=PUTSECURITY,4=READREGION,5=PUTREGION,6=READCOUNTER,7=PUTCOUNTER)
        ...
        .else
            ...
            .endif
        .endif
        mov bMinimizeWindowIntoNotificationIcon,al
        ;
        ;¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬ READ: ArchiveMode ¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬
        .if bUsarRegistro==FALSE
    ...
    .elseif bAction == 1 || bAction == 2;(WinHandle),(0=GET,1=PUTMAIN,2=READMAIN,3=PUTSECURITY,4=READREGION,5=PUTREGION,6=READCOUNTER,7=PUTCOUNTER)
        ...
        .endif
        ;
        ;¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬ WRITE:  Minimize Window Into Notification Icon ¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬¬
        Invoke GetDlgItem,hWnd,IDC_CHK_MINIMIZE_AT_SYSTEM_TRAY
        Invoke  SendMessage, eax, BM_GETCHECK, 0, 0
        .if eax==BST_CHECKED
            mov bMinimizeWindowIntoNotificationIcon,TRUE
        .else
            mov bMinimizeWindowIntoNotificationIcon,FALSE
        .endif
        movzx eax,bMinimizeWindowIntoNotificationIcon
        .if bAction == 1
            .if bUsarRegistro==FALSE
                Invoke wsprintf,addr szBuffer,addr szIntFormat,eax
...
; «««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««««
align 4
GetRegistryMouseMenu            proc pszMouseHoverTime:DWORD,pszMenuShowDelay:DWORD
                        ;
                        LOCAL dResult:DWORD
                        LOCAL hReg:DWORD
                        LOCAL dSize:DWORD
...
;                Invoke ShowWindow,hWinInTurn,SW_SHOW
;                Invoke SetForegroundWindow,hWinInTurn
;                Invoke SetWindowPos,hWinInTurn,NULL,0,0,0,0,SWP_NOMOVE OR SWP_NOSIZE OR SWP_NOZORDER OR SWP_FRAMECHANGED
                ;
                ...
                ;
                ;Invoke MessageBox,NULL,addr szProgramPath,NULL,NULL
                ;Invoke ShowWindow,hWinInTurn,SW_SHOWNORMAL
                ;Invoke ShowWindow,hWinInTurn,SW_SHOW
                ;Invoke Sleep,10
                ;Invoke SetForegroundWindow,hWinInTurn
                ;Invoke ForceWindowToTop,hWinInTurn
...
;                ;Invoke IsIconic,hWinInTurn
                ;.if eax
                ;    Invoke ShowWindow,hWinInTurn,SW_RESTORE
                ;    ;Invoke ShowWindow,hWinInTurn,SW_SHOWNORMAL or SW_RESTORE
                ;    Invoke ShowWindow,hWinInTurn,SW_SHOW
                ;    Invoke SetForegroundWindow,hWinInTurn
                ;.else
                ;    ;Invoke ShowWindowAsync,hWinInTurn, SW_SHOWNORMAL
                ;    Invoke ShowWindow,hWinInTurn,SW_SHOW
                ;.endif
                ;Invoke SetForegroundWindow,hWinInTurn
                ;Invoke ForceWindowToTop,hWinInTurn
